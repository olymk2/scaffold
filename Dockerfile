...
RUN apk add --update --no-cache build-base make python3-dev jpeg-dev zlib-dev libffi-dev openssl-dev libxml2-dev libxslt-dev && \
    pip3 install lxml && \
    pip3 install --no-cache-dir lxml dateutils requests requests-oauthlib mailer pillow gocardless paypalrestsdk pytz nose2 test_utils pytest oauthlib flask flask-login pymysql misaka slimit minify cssmin && \
