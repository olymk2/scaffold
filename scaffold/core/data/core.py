...
class db_sqlite(base):
    def tables(self):
        self.query_str = "select tbl_name from sqlite_master where type='table'"
        for row in self.__iter__():
            yield row

    def columns(self):
        for table_name in self.tables():
            self.query_str = 'pragma table_info(%s)' % table_name[-1]
            for row in self.__iter__():
                yield row
...
class db_mysql(base):
    ...
    def database(self):
        self.query_str = "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.COLUMNS"

        for row in self.__iter__():
            yield row

    def tables(self, data):
        self.required = {'TABLE_SCHEMA'}
        self.columns_where = {'TABLE_SCHEMA'}
        self.grouping = ['TABLE_NAME']
        self.query_str = "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.COLUMNS"
        self.set(data)
        for row in self.__iter__():
            yield row.get('TABLE_NAME')

    def columns(self,data):
        self.required = {'TABLE_SCHEMA', 'TABLE_NAME'}
        self.columns_where = {'TABLE_SCHEMA', 'TABLE_NAME'}
        self.grouping = {}
        self.query_str = "SELECT * FROM INFORMATION_SCHEMA.COLUMNS"
        self.set(data)
        for row in self.__iter__():
            yield row.get('COLUMN_NAME')

class get_database_info(select_data):
...
class get_table_columns(select_data):
    debug = True
...
class get_table_column_indexes(select_data):
    required = {'TABLE_SCHEMA', 'TABLE_NAME'}
    count_rows = False
    limit_enabled = False
