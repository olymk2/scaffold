...
class control(html_form_ui):        
    ...
    def create(self,name, dict={}, error=None):
        ...
        val=""
        form+=self.form_node_wrap("<input sa:valformat=\""+val+"\" class=\"formtext\" style=\""+style+"\" id=\""+name+"\" name=\""+name+"\" maxlength=\""+str(length)+"\" value=\""+str(value[0])+"\" />"+imgenum,name,title,null=True,msg=error_msg)
        ...
        for num in range(1,int(enum)):
            ...
            form+=self.form_node_wrap("<input sa:valformat=\""+val+"\" class=\"formtext\" style=\""+style+"\" id=\""+newname+"\" name=\""+newname+"\" maxlength=\""+str(length)+"\" value=\""+str(value[num])+"\" />"+imgenum,newname,"",null=True,msg=error_msg)
        form+="</div>"
